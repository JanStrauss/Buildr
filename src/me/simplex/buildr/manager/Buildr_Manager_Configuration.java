...
public static String getMsg(String msg)  // New method, with an argument. You need to specify the argument to use this method
	{
		Properties prop = new Properties(); //Creates The Properties file
		
		try {
			if (!new File("plugins/Buildr/version.properties").exists()) //Try to see if it exists
			{
				new File("plugins/Buildr").mkdir();// If the directory doesn't exist, make one
				new File("plugins/Buildr/version.properties").createNewFile(); // if it doesn't, create a new one
				PrintWriter out = new PrintWriter(new FileWriter("plugins/Buildr/version.properties")); //Specify the path of the new file
				out.println("# DO NOT edit this file!");
				out.println("# This will mean that you do not have to delete your config file when updating");
				out.println("Version=0.5");
				out.close();// Close the output stream
			}	
			  FileInputStream in = new FileInputStream("plugins/Buildr/version.properties");  // Open an input stream
			    prop.load(in); // Loads the properties from the input stream
			   
				String iv = prop.toString(); // Make the argument (item) a string
				String[] test = iv.split("="); //split on the = because toString gets the whole line
				
				String[] test2 = test[1].split(" "); //Split that with whitespaces as it surround the line with braces
			    	...
			    	System.out.println("[Buildr] Error while reading version.properties!"); 
			    	e.printStackTrace();
			    }
		return msg;
