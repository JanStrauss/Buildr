...
public class Buildr_Manager_Builder_Cuboid implements Buildr_Interface_Building {
	...
	public Buildr_Manager_Builder_Cuboid(Player wallcreater,Material material, boolean replace,Material replace_mat,boolean hollow, Buildr plugin, byte material_data) {
	...
	public void startBuild() {
		plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Buildr_Runnable_Builder_Cuboid(position1, position2, material, replace, replace_mat
				,hollow, plugin, wallcreater,material_data));
	}
}
